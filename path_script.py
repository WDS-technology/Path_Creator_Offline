...
class FlightPlannerApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.command_buttons = {
            ...
            "Set New Yaw": "SCHEDULE_FLY_TO_YAW",
            "Take Picture": "SCHEDULE_TAKE_PICTURE",
            "Start Recording": "SCHEDULE_SET_PAYLOAD_RECORDING",
        }
        for label, command in self.command_buttons.items():
            btn = ttk.Button(
                self.command_frame,
                text=label,
                command=lambda cmd=command: self.add_command(cmd),
            )
            btn.pack(side=tk.TOP, pady=5)  # Stack buttons vertically
    ...
    def draw_grid(self):
        # Draw 10x10 grid
        for i in range(11):  # 10 lines in each direction
            self.canvas.create_line(
                i * 50, 0, i * 50, 500, fill="gray"
            )  # Vertical lines
            self.canvas.create_line(
                0, i * 50, 500, i * 50, fill="gray"
            )  # Horizontal lines
        # Draw center point
    ...
    def add_command(self, command):
        if len(self.path_points) == 0:  # If there are no points on the canvas
            return

        # Last point where this command will be added
        x, y = self.path_points[-1]["canvas_coordinates"]
